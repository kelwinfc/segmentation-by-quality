...
class PixelDCN(BaseNetwork):
    def build_network(self, X=None, y=None):
        ...
        for conv_level in range(self.conv_num)[::-1]:
            ...
            #deconv = KL.Conv2DTranspose(sizes[conv_level],
            #                            self.pool_size,
            #                            strides=(self.pool_size,
            #                                     self.pool_size))(last_)
            deconv = KL.core.Lambda(lambda x: ops.ipixel_dcl(
                x, sizes[conv_level], self.pool_size,
                'ipixel-deconv-%d' % conv_level, activation_fn=tf.nn.relu))(last_)
    ...
    def get_generator(self, path, subset):
        ...
        return super(PixelDCN, self).get_generator(path, subset)
